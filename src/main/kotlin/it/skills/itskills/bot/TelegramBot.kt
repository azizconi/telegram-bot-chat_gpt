...
import it.skills.itskills.PROJECT_PATH
...
import it.skills.itskills.bot.utils.AudioConverterExtension
...
//            }
            ...
            if (message.hasText()) {
                ...
                println("message.voice = ${message.voice != null}")
                when {
                    message.text == "/start" -> {
                        sendTextContent(
                            chatId,
                    ...
                    message.text.startsWith("#voice ") -> {
                        val realMessage = message.text.substring(7)
                            ...
                            when (result) {
                                is Resource.Success -> {
                                    ...
                                    repository.textToSpeech(
                                        TextToSpeechRequestModel(input = result.data?.choices?.get(0)?.message?.content!!)
                                    ).collect {
                                        ...
                                        when (it) {
                                            ...
                                            is Resource.Success -> {
                                                ...
                                                it.data?.let { responseBody ->
                                                    ...
                                                    val inputFilePath =
                                                        "$PROJECT_PATH/files/voice/original/$chatId$random.mpga"
                                                    val outputFilePath =
                                                        "$PROJECT_PATH/files/voice/telegram/$chatId$random.ogg"
                                                    val file = java.io.File(inputFilePath)
                                                    FileOutputStream(file).use { outputStream ->
                                                        responseBody.byteStream().use { inputStream ->
                                                            AudioConverterExtension.copyStream(
                                                                inputStream,
                                                                outputStream
                                                            )
                                                        }
                                                    ...
                                                    AudioConverterExtension.executeFFmpegCommand(
                                                        inputFilePath,
                                                        outputFilePath
                                                    )

                                                    val sendVoice = SendAudio()
                                                    sendVoice.chatId = chatId

                                                    val voiceFile = java.io.File(outputFilePath)
                                                    sendVoice.audio = InputFile(voiceFile)
                                                    execute(sendVoice)
...
//                        )
    ...
    fun textResponseForUser(
        result: Resource<ChatGptResponseModel>,
        ...
        result.data?.choices?.let {
            if (it.isNotEmpty()) {
                ...
                try {
                    val userEntity = service.getUserById(userId)
